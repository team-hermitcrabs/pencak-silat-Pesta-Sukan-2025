...
const HUBSPOT_SERVERLESS_ENDPOINT =
	"https://www.activesgcircle.gov.sg/_hcms/api/update-data-sport-silat";
const HUBDB_ROW_ID_COLUMN = 1; // Column A for storing HubDB row ID (hs_id)
const SYNC_STATUS_ID_COLUMN = 13; // Column M for storing sync status
const SYNC_MESSAGE_ID_COLUMN = 14; // Column N for storing sync message
...
function getAllSheetData(sheet, lastRow) {
	...
	values.forEach((row, index) => {
		...
		const data = {
			...
			blue_score: row[5] || "",
			blue_corner: row[6] || "",
			blue_corner_club: row[7] || "",
			red_corner: row[8] || "",
			red_corner_club: row[9] || "",
			red_score: row[10] || "",
			hubdbRowId: hubdbRowId,
			sheetRow: actualRow,
		};
...
function findRowsNeedingHubDBCreation(allData, sheet, changes) {
	...
	changedUniqueIds.forEach((uniqueId) => {
		...
		const hasRequiredData =
			data.date_and_time && data.blue_corner && data.red_corner;
		...
		for (const rowInfo of rowsNeedingCreation) {
			...
			const payload = {
				...
				data: {
					...
					arena: rowInfo.data.arena,
					round: rowInfo.data.round,
					blue_score: rowInfo.data.blue_score,
					blue_corner: rowInfo.data.blue_corner,
					blue_corner_club: rowInfo.data.blue_corner_club || "",
					red_corner: rowInfo.data.red_corner || "",
					red_corner_club: rowInfo.data.red_corner_club,
					red_score: rowInfo.data.red_score,
				},
				metadata: {
...
function handleBatchDeleteSuccess(deleteChanges, sheet, responseText) {
	...
	if (batchResponseData.success) {
		const successfulDeletions = batchResponseData.results?.filter(
			(r) => r.status === "success"
		) || [];
